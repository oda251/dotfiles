---
name: journal
description: "Obsidian ジャーナル・ドキュメント記録。調査、コーディング等あらゆるタスクで細かく記録を残す。例: タスクの区切り / 手戻りや想定外の事象 / 方針転換 / ミス＋リカバリ / 技術的発見 / 非自明な判断 / 再利用できそうな知見。該当する moment を検出したら**次のツール呼び出し前に**append する（実作業より優先、後回しは揮発性ゆえ実質スキップ）。"
inputs:
  content: 書き込む内容
---
...
- **moment 検出時は次のツール呼び出し前に append する（実作業より優先）**。対象例: 方針転換 / ミス＋リカバリ / 技術的発見 / 外部資料を参照して判断を変えた / 削除・スコープ縮小の判断。「次のターンで書こう」「まとめて後で書こう」は揮発性ゆえ実質スキップになるので禁止。1 行 append は 2-3 秒でクリティカルパスにならない
