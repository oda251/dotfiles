...
tags:
  ...
  - mcp
---
# juggler — エージェントワークフローオーケストレーター

## 概要

juggler は Claude Code メインエージェントとワーカーエージェントの間に立つオーケストレーター。
MCP サーバーとしてデーモン動作し、ワークフロー定義に基づいてタスクの委譲・チェーン実行・通知を行う。

## アーキテクチャ

```
CC メインエージェント ←MCP(ツール+Channel)→ juggler daemon ←CLI→ ワーカーエージェント
```

- **メインエージェント**: ユーザーと対話し、タスクを分解して juggler に投入する（plan）
- **juggler**: MCP サーバーとしてツール提供 + Channel で通知。ワーカーの起動・管理を行う
...
- juggler は CC セッションを跨いでデーモンとして動作する
...
5. juggler が前ステップの inputs + outputs を次ステップの inputs として渡す
...
juggler がワーカーを起動する際、以下をプロンプトとして注入する:
...
3. 実行する（ワークフロー本文に従う）
4. セルフレビュー
5a. OK → juggler done --output key=value ...
