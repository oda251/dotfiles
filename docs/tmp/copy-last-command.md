...
- [ ] R2 出力なし: `❯ true` の直後に次プロンプト → `$ true` の1行のみを返す（末尾に空行を残さない）
- [ ] R3 履歴不一致時のフォールバック: 直前コマンド文字列に一致する行が無ければ、末尾の `cpl` 行を読み飛ばして直近の非 cpl コマンド行を対象にし、コマンド文字列はペインの行から取る
- [ ] R4 出力中の偽プロンプト: 出力行が `❯ ` で始まっていても、切り出しの終端は自分自身（`cpl`）の実行行とし、偽プロンプト行を出力に含める
- [ ] R5 プロンプト行なし: `❯` 行が1つも無いテキストでは非0終了し、何も出力しない
- [ ] R6 直接確認: herdr 外（`HERDR_ENV` 未設定）では `cpl` がエラーメッセージを出して非0終了し、クリップボードを触らない
- [ ] R7 直接確認: 実ペインで `cpl` を実行すると、クリップボードに `$ <コマンド>` + 出力が入り、コピー行数のサマリが表示される
