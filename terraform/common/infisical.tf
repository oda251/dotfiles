...
locals {
  ...
  user_secrets = {
    ...
    INFISICAL_GHA_CLIENT_ID     = { folder = "/terraform", comment = "Infisical Universal Auth client ID for GitHub Actions" }
    INFISICAL_GHA_CLIENT_SECRET = { folder = "/terraform", comment = "Infisical Universal Auth client secret for GitHub Actions" }
  }
}

variable "infisical_gha_identity_id" {
  description = "Infisical Machine Identity ID for GitHub Actions (created manually in UI)"
  type        = string
  default     = ""
}
...
resource "infisical_project_identity" "github_actions" {
  count       = var.infisical_gha_identity_id != "" ? 1 : 0
  project_id  = var.infisical_project_id
  identity_id = var.infisical_gha_identity_id
  roles = [
    {
      role_slug = "viewer"
    }
