...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  if [[ -n "$toplevel" ]] && command -v gwq >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    worktree_path="$(gwq list -g --json 2>/dev/null \
      | jq -r --arg p "$toplevel" 'map(select(.path == $p)) | .[0].path // ""' 2>/dev/null || true)"
  fi

  if command -v gh >/dev/null 2>&1; then
    repo_nwo="$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null || true)"
    if [[ -n "$branch" ]] && command -v jq >/dev/null 2>&1; then
      pr_raw="$(gh pr list --head "$branch" --state all \
        --json number,state,url,mergedAt,title 2>/dev/null || echo '[]')"
      pr_json="$(printf '%s' "$pr_raw" | jq 'if length == 0 then null else .[0] end' 2>/dev/null || echo null)"
    fi
  fi
fi
